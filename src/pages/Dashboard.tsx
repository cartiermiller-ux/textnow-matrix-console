...
import { STATUS_MAP, TASK_STATUS_MAP } from "@/lib/constants";
...
export default function Dashboard() {
  ...
  const statCards = [
    { label: "号池总量", value: stats.total, icon: Users, color: "text-blue-600 bg-blue-50", sub: `可用 ${stats.available}` },
    { label: "今日发送量", value: stats.todaySent, icon: Send, color: "text-green-600 bg-green-50", sub: `成功率 ${stats.successRate}%` },
    { label: "今日接收量", value: stats.todayReceived, icon: Inbox, color: "text-purple-600 bg-purple-50", sub: `${conversations.length} 个会话` },
    { label: "封禁率", value: `${stats.banRate}%`, icon: AlertTriangle, color: "text-red-600 bg-red-50", sub: `封禁 ${stats.banned}` },
  ];
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className={`font-mono ${stats.successRate >= 80 ? "text-green-600" : "text-amber-600"}`}>{stats.successRate}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">待续期账号</span>
              <span className={`font-mono ${stats.pendingRenew > 0 ? "text-amber-600" : "text-green-600"}`}>{stats.pendingRenew}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">封禁账号</span>
              <span className={`font-mono ${stats.banned > 0 ? "text-red-600" : "text-green-600"}`}>{stats.banned}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">活跃会话</span>
              <span className="font-mono">{conversations.filter(c => c.unread > 0).length}</span>
            </div>
          </div>
        </Card>
      </div>
